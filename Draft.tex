...
using province-level vehicle sales data in China from 2019 to 2023. 
EV demands under indirect network effects are examined for evaluation of policy impacts on EV adoptions.
Part of calculated price elasticities exhibits increased complementarity across models, confirm the presence of significant indirect network effects, 
...
Targeted policies on purchase price of EVs exhibits the most significant impact when considering only EV adoption.
In particular, the phased-out uniform purchase subsidy is more cost-effective than non-uniform subsidies such as tax exemptions and range-based incentives,
...
leveraging the presence of feedback effects caused by the indirect network externalities between the two sides \citep{kellogg_energy_2021}. 
...
In particular, with estimated a negative price estimate, vehicle models act as substitutes without network externalities. 
...
But EV penetration was unsuccessful during earlier stages due to immature production technology, lack of scale and insufficient support from infrastructures. 
...
In the meantime, local governments following central guidelines, began to initiate disaggregated policies favouring EVs over traditional ICE vehicles. 
...
variants in the model such as the principles-of-differentiation general extreme-value (PD-GEV) model, random-coefficient discrete choice model in \citet{berry_automobile_1995}, the adopted approach in this paper of nested logit model (\citep{cardell_variance_1997}). 
...
\citet{springel_network_2021} asked a similar question in the Norwegian market, but implemented a structural demand-supply equilibrium setup and estimated a random coefficient model under the influence of indirect network externality of charging stations, 
...
As for studies on the Chinese automobile industry, \citet{barwick_attribute-based_2024} studied the impact of attribute-based incentives to EV adoption building on a structural demand model. 
...
in which the presence of indirect network effect would generally alter policy implications significantly \citep{rapson_economics_2021}. 
...
which was then extended to a formal two-sided 'platform' framework by later works studies such as \citet{rochet_platform_2003}, \citet{wright_one-sided_2004}, and \citet{armstrong_competition_2006}. 
...
and \citet{reshef_smaller_2023} in online takeout platform, have identified material conclusions of how positive indirect network alters competition.
As for the subset of this literature that applies specifically to EV, following \citet{li_market_2017} and \citet{springel_network_2021}, recent studies have all taken indirect network effect into account when considering EV penetration.
...
\section{Data}
\hspace*{\parindent}The data I use is combiled from a variety of sources, including a commercial source that aggregated registry data from the Chinese Association of Automobile Manufacturers (CAAM), 
...
\section{Modelling}
    ...
    \hspace*{\parindent}For modelling vehicle demand side, I follow the tradtitional setup of differentiated product demand estimation to use a random utility maximization setup for modelling of discrete choices \citep{berry_estimating_1994, gandhi_empirical_2021}. 
...
which facilitates the recovery of realistic policy implications. Due to limitation in time and resources of this study, I adopt a nested logit framework which can achieve similar goals with less computational burden \citep{grigolon_nested_2014}.
...
and as shown by \citet{cardell_variance_1997}, there exist a unique distribution of $\zeta_{ig}$ that $[\zeta_{ig} + (1-\rho)\epsilon]$ also follows an extreme value distribution. 
...
In the spirit of \citet{berry_estimating_1994}, I write the well-known result of choice probability of choosing product $j$ within group $g$, which equivalent gives the within-group market of product $j$:
...
The differentiation IVs quantifies the level of competition faced by a product through some measures of its position in the product characteristics space.
...
a commonly known feature of the automobile manufacturers is that frame designs for a model is determined at the beginning of the production cycle \citep{barwick_attribute-based_2024},
...
\hspace*{\parindent} I now run a set of policy counterfactual analyses based on the estimation results. There are mainly three sets of policy questions I am interested in answering.
